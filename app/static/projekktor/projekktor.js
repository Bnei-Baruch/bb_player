/*!
 * Projekktor II - HTML5 Media Player, Projekktor Core Version: V1.2.20 r194
...
jQuery(function ($) {
    ...
    projekktor = $p = function () {
        ...
        function PPlayer(srcNode, cfg, onReady) {
            this.config = new projekktorConfig("1.2.20");
            this.env = {
                muted: false,
                playerDom: null,
                mediaContainer: null,
...
jQuery(function (a) {
    playerModel.prototype = {
        ...
        setFrame: function (c) {
            var b = (c / this.pp.getConfig("fps")) + 0.00001;
            this.setSeek(b)
        },
        setSeek: function (b) {},
        setPlay: function () {},
        ...
        timeListener: function (d) {
            ...
            var b = parseFloat((d.position || d.currentTime || this.media.position || 0).toFixed(2)),
                c = parseFloat((d.duration || 0).toFixed(2));
            if (isNaN(c + b)) {
                return
            }
            if (c != 0 && (c != this.media.duration && !this.isPseudoStream) || (this.isPseudoStream && this.media.duration == 0)) {
                this.media.duration = c;
                this.sendUpdate("durationChange", c)
            }
    ...
    projekktorControlbar.prototype = {
        ...
        displayVolume: function (f) {
            ...
            if (this.controlElements.vslider.width() > this.controlElements.vslider.height()) {
                ...
                this.controlElements.vmarker.css("height", f * 100 + "%");
                this.controlElements.vknob.css("bottom", f * 100 + "%")
            }
            if (c) {
                switch (parseFloat(f)) {
                    case 0:
        ...
        vsliderClk: function (c) {
            ...
            var g = a(this.controlElements.vslider),
                d = g.width() > g.height() ? "hor" : "vert",
                b = (d == "hor") ? g.width() : g.height(),
                f = (c.originalEvent.touches) ? c.originalEvent.touches[0].pageX : c.pageX,
                e = (c.originalEvent.touches) ? c.originalEvent.touches[0].pageY : c.pageY,
                h = (d == "hor") ? f - g.offset().left : e - g.offset().top;
            if (h < 0 || isNaN(h) || h == undefined) {
                result = 0
            } else {
                result = (d == "hor") ? (h / b) : 1 - (h / b)
            }
...
jQuery(function (a) {
    projekktorContextmenu.prototype = {
        ...
        initialize: function () {
            var b = this,
                c = this.pp.getIframeWindow() || a(window);
            ...
            if (this.pp.getConfig("helpHome")) {
                this._items.help = {
                    getContextTitle: function () {
                        return b.pp.getConfig("messages")[100]
                    },
                    open: function () {
                        b.popup(b.pp.getConfig("helpHome"), 400, 600)
                    }
                }
            }
            this.pluginReady = true
        ...
        popup: function (c, d, b) {
            centeredY = window.screenY + (((window.outerHeight / 2) - (b / 2)));
            centeredX = window.screenX + (((window.outerWidth / 2) - (d / 2)));
            window.open(c, "projekktor", "height=" + b + ",width=" + d + ",toolbar=0,scrollbars=0,status=0,resizable=1,location=0,menuBar=0,left=" + centeredX + ",top=" + centeredY).focus()
        }
    }
});
